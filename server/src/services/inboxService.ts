...
export const inboxService = {
  ...
  async getCommunications(params: { userId?: string; timeframe?: 'This Month'|'Last Month'|'This Quarter'; type?: 'EMAIL'|'SMS'|'CALL'; leadOnly?: boolean }) {
    const now = dayjs();
    ...
    return inboxRepository.listCommunications({ userId: params.userId, from: start?.toDate(), to: end?.toDate(), type: params.type, leadOnly: params.leadOnly });
  }
};
